...
- 基于源码版本运行Demo应用(独立Server版本)
```
$ npm run start:src 
```

- 基于源码版本运行Demo应用(ExpressRouter路由挂载版本)
```
...
# 运行较复杂的示例项目：
按以下步骤运行源码中较复杂的示例项目，它带有sequelize-ORM、API自动缓存、管理身份与权限校验等高阶场景
- 在本地创建名为gankao_demo123的mysql数据库
- 执行数据库初始化脚本
```
$ FORCE=0 DB_RESET_1=123234537569 DB_RESET_2=wrq5hfoiuy12344376 node toolscript/resetDB.run.js
```
- 在源码根目录下，进入node示例项目并执行
```
...
```
另外，源码中还有一个初级的示例项目，位于/example/baseProject
...
## 空间列表
- class2api             主空间，提供微服务创建、缓存修饰器、redis配置/访问等基础套件
- class2api/dbhelper    sequelize模型创建与管理的助手函数空间，可简化model的声明与引用
- class2api/rulehelper  管理后台级的身份、权限校验
- class2api/testhelper  测试相关的辅助套件，快速执行基于http接口调用的单元测试，以及收集单元测试的请求与结果json快照，并在mocha的after拦截中保存到指定文件，以作为文档参考
- class2api/gkerrors    常用的预设错误类型
...
```javascript
    createServer({
        ...
        custom:(expressInstence)=>{
            //TODO:对expressInstence实例进行自定义扩展，注：微服务通常是无状态的，所以不建议增加session机制
...
创建服务器(使用外部的Express，只返回router供绑定路由)，opts参数与createServer函数基本相同，除了自动忽略custom参数。可参考项目中的server-router.js源码
...
## class2api/gkerrors   常用预设的错误（code值为负数）
- GKErrors  预设错误有：
```javascript
...
类静态方法修饰器,提供权限校验，同时为当前修饰的类静态方法标记了权限名称、描述信息
...
exports.config = { 
    name: 'courseService', 
    admin_rule_center: { 
        auth:"http://127.0.0.1:3002/gkrulemanager/auth", //身份验证接口
        validator: "http://127.0.0.1:3002/gkrulemanager/validate",  //权限验证接口
...
class2api内置的身份校验函数，函数内将jwtoken参数会以header的方式发送给class2api.config.js中指定的config.admin_rule_center.auth接口，auth接口端返回详细的用户信息（按约定，用户信息中含有唯一的uID属性）
...
向api发起一个post-API请求， postParams是post的JSON内容，apiDesc是API方法描述的函数封装（只是提高可读性，没有实际功能意义）
