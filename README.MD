    ...
    createServer(ClassA)
        .then((server)=>{
            server.listen(3002, "0.0.0.0", (err)=>{
                ...
                console.info("==> API Started, test for $ curl -d 'name=huangyong' 'http://127.0.0.1:3002/ClassA/hello'.");
            });
        })
``` 
...
```
另外，源码中还有一个初级的示例项目可供参考，位于/example/baseProject
...
- class2api/rulehelper  管理后台专用的身份检验、权限校验
